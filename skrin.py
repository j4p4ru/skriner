...
# =============================================================================
st.set_page_config(
    page_title="Quant Trader - IDX Screener Ultra v6.0",
    layout="wide",
...
def snap_to_tick(price: float, direction: str = "floor") -> int:
    ...
    FIX #1: Tabel lama punya tier fiktif Rp50 untuk harga >=10.000 — TIDAK
    ADA dalam regulasi BEI manapun. Aturan resmi (Kep-00023/BEI/04-2016,
    dikonfirmasi masih berlaku per Kep-00055/BEI/03-2023) hanya 5 tier,
    dan tier teratas (>=Rp5.000 = Rp25) berlaku TANPA BATAS ATAS — saham
    seharga Rp50.000 pun tetap fraksi Rp25, bukan Rp50.

    Fraksi harga IDX (BEI) yang BENAR:
    < 200        → Rp1
    200 – 500    → Rp2
    500 – 2.000  → Rp5
    2.000 – 5.000→ Rp10
    >= 5.000     → Rp25  (tidak ada tier lebih lanjut)
    ...
    if p < 200:
        ...
        tick = 25     # FIX #1: tidak ada lagi tier Rp50
...
def build_trade_plan(last_close: float, atr_val: float, mode_params: dict) -> dict:
    ...
    FIX #2 (kritis): buy_max kini di-snap CEIL (bukan floor). Versi lama
    men-floor-snap buy_max dari last_close, sehingga ketika live price gagal
    di-fetch dan fallback ke last_close persis, harga itu HAMPIR SELALU
    berakhir di ATAS buy_max hasil snap — status jadi "waiting/chasing"
    padahal seharusnya valid. Dengan ceil-snap, last_close dijamin
    buy_min <= last_close <= buy_max selama guard lain tidak memotongnya.
    ...
    # ── Zona beli — FIX #2: buy_max ceil-snap agar mencakup last_close ──────
    ...
    buy_max = snap_to_tick(buy_max_raw, "ceil")        # dulu "floor" — BUG

    tick = idx_tick_of(last_close)
...
def idx_tick_of(price: float) -> int:
    """Kembalikan ukuran tick (bukan harga ter-snap) untuk harga tertentu."""
    p = float(price)
    if p < 200: return 1
    if p < 500: return 2
    if p < 2000: return 5
...
def quant_strategy_engine(all_data: dict, config: dict, trading_mode: str) -> pd.DataFrame:
    ...
    def _validity_rank(row) -> int:
        lp, sl   = row.get("Live Price", 0), row.get("Stop Loss", 0)
        bmin, bmax = row.get("Buy Min", 0), row.get("Buy Max", 0)
        if lp <= sl:        return 2
        elif lp < bmin:     return 1
        elif lp > bmax:     return 1
        else:               return 0
